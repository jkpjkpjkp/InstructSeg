...
def eval_queue(q, rank, out_dict, revos_pred_path):
    while not q.empty():
        vid_name, exp = q.get()
        ...
        try:
            ...
            if not os.path.exists(f'{revos_pred_path}/{vid_name}'):
                print(f'{vid_name} not found')
                out_dict[exp_name] = [0, 0, 0, 0]
                continue

            pred_0_path = f'{revos_pred_path}/{vid_name}/{exp}/{vid["frames"][0]}.png'
            ...
            for frame_idx, frame_name in enumerate(vid['frames']):
                ...
                pred_masks[frame_idx] = cv2.imread(f'{revos_pred_path}/{vid_name}/{exp}/{frame_name}.png', cv2.IMREAD_GRAYSCALE)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--revos_pred_path", type=str, default='output/revos')
    parser.add_argument("--revos_exp_path", type=str, default="dataset/ReVOS/meta_expressions_valid_.json")
    parser.add_argument("--revos_mask_path", type=str, default="dataset/ReVOS/mask_dict.json")
    parser.add_argument("--revos_foreground_mask_path", type=str, default="dataset/ReVOS/mask_dict_foreground.json")
    ...
    queue                = mp.Queue()
    exp_dict             = json.load(open(args.revos_exp_path))['videos']
    mask_dict            = json.load(open(args.revos_mask_path))
    mask_dict_foreground = json.load(open(args.revos_foreground_mask_path, 'r'))
    ...
    if NUM_WOEKERS > 1:
        ...
        for rank in range(NUM_WOEKERS):
            p = mp.Process(target=eval_queue, args=(queue, rank, output_dict, args.revos_pred_path))
            p.start()
            processes.append(p)
        ...
        eval_queue(queue, 0, output_dict, args.revos_pred_path)

    # save average results
    output_json_path = osp.join(args.revos_pred_path, args.save_json_name)
    output_csv_path  = osp.join(args.revos_pred_path, args.save_csv_name)
